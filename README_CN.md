...
[![Version](https://img.shields.io/badge/version-2.5-blue.svg)](https://github.com/myths-labs/muse/blob/main/CHANGELOG.md)
...
**Q: MUSE 需要安装吗？**
不需要。MUSE 是纯 Markdown 文件。复制到你的项目即可使用。零依赖。

**Q: 支持哪些 AI 工具？**
任何支持 system prompt / project rules 的工具：Claude Code（`CLAUDE.md`）、Cursor（`.cursorrules`）、Windsurf、Copilot 等。

**Q: 和 lossless-claw 有什么区别？**
lossless-claw 是代码插件（SQLite + DAG + 子代理），依赖 OpenClaw 运行时。MUSE 是纯 Markdown SOP，适用于任何 AI 工具，零依赖。相同理念，不同实现。
